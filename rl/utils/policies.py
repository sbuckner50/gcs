...
class PolicyAutoRegressiveModel(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_dim=65, hidden_depth=2, num_buckets=10, ac_low=-1, ac_high=1):
        ...
        self.ac_low = torch.tensor(ac_low).float().to(device)
        self.ac_high = torch.tensor(ac_high).float().to(device)
        self.num_buckets = num_buckets
        self.bucket_size = torch.tensor((ac_high - ac_low) / num_buckets).float().to(device)
    ...
    def forward(self, state):
        vals = torch.tensor([]).float().to(device)
...
def rollout(
    ...
    while path_length < episode_length:
        ...
        o_for_agent = o
        o_for_agent = torch.from_numpy(o_for_agent[None]).float().to(device)
        action, _ = agent(o_for_agent)
        action = action.cpu().detach().numpy()
        
        # Step the simulation forward
        next_o, r, done = env.step(action.reshape(-1,1))
        ...
        # Terminate rollout early if needed
        if done:
            break
        if env.evaluate_runaway_error(o.reshape(-1,1)):
            print("Infeasible observation detected in rollout, terminating rollout early..")
            break
        if np.isnan(o).any():
            print("Error: NaNs detected in observation, terminating rollout early..")
            break
